...
class AppealGraph(BaseStateGraph):
    ...
    async def _node_awaiting_documents(self, state: Dict) -> Dict:
        ...
        state.setdefault("documents_received", [])
        ...
        # If this is a resumed run (HITL already resolved), the admin's
        # decision carries the documents that came in. Apply them and move on.
        decision = state.get("hitl_decision", {}) or {}
        if decision:
            state["documents_received"] = decision.get("documents", state["documents_received"])
            state["document_status"] = "received"
            state["documents_received_at"] = datetime.now().isoformat()
            state["appeal_steps"].append("documents_received_via_hitl")
            return {"next": "documents_received"}

        return {"next": "awaiting_documents"}  # Will pause here (first pass, pre-HITL)
    ...
    async def _node_underwriter_review(self, state: Dict) -> Dict:
        ...
        if decision:
            ...
            state["appeal_steps"].append("underwriter_review_resolved")
            approved = decision.get("approved", True)
            return {"next": "appeal_approved" if approved else "appeal_denied"}
    ...
    async def _node_escalated_to_manager(self, state: Dict) -> Dict:
        ...
        if decision:
            ...
            state["escalation_resolved_at"] = datetime.now().isoformat()
            state["appeal_steps"].append("escalation_resolved")
            approved = decision.get("approved", True)
            return {"next": "appeal_approved" if approved else "appeal_final_denied"}

        state["escalation_status"] = "pending"
        return {"next": "escalated_to_manager"}  # Will pause here (first pass, pre-HITL)
