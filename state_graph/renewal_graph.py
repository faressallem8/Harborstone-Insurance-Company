...
class RenewalGraph(BaseStateGraph):
    ...
    def get_hitl_conditions(self) -> List[str]:
        ...
        return [
            "await_inspection_report",  # Pauses until an admin submits the report (or marks it unavailable)
    ...
    async def _node_await_inspection_report(self, state: Dict) -> Dict:
        """
        Wait for vessel inspection report (HITL pause).
        ...
        The graph pauses here until an admin either:
        1. Submits the inspection report → report_received → risk_assessment
        2. Marks it as timed out / unavailable → report_timeout (creates a ticket)
        """
        state["inspection_requested_at"] = state.get(
            "inspection_requested_at", datetime.now().isoformat()
        )
        state["inspection_status"] = "pending"
        state["inspection_expected_by"] = state.get(
            "inspection_expected_by",
        ...
        if decision:
            if decision.get("timed_out"):
                state["renewal_steps"].append("inspection_marked_timed_out")
                return {"next": "report_timeout"}

            report = decision.get("inspection_report") or state.get("inspection_report", {})
            state["inspection_report"] = report
            state["renewal_steps"].append("inspection_report_submitted")
            return {"next": "report_received"}

        # No decision yet - main loop will pause here (first pass, pre-HITL)
        return {"next": "await_inspection_report"}
    ...
    async def _node_report_timeout(self, state: Dict) -> Dict:
        ...
        Reached when an admin marks the inspection as unavailable/timed out
        (see _node_await_inspection_report). The admin can then, via the
        resulting ticket:
        1. Manually enter the inspection data
        2. Request the inspection again
        3. Cancel the renewal
        """
        error = "Inspection report was not received (marked timed out by admin)"
        ticket_id = self._create_ticket("await_inspection_report", state, error)

        state["inspection_status"] = "timeout"
        state["timeout_at"] = datetime.now().isoformat()
        state["timeout_ticket_id"] = ticket_id
    ...
    async def _node_underwriter_review(self, state: Dict) -> Dict:
        ...
        if decision:
            ...
            state["underwriter_reviewed_at"] = datetime.now().isoformat()
            state["renewal_steps"].append("underwriter_review_resolved")
            approved = decision.get("approved", True)
            return {"next": "renewal_approved" if approved else "renewal_denied"}

        state["review_status"] = "pending"
        return {"next": "underwriter_review"}  # Will pause here (first pass, pre-HITL)
